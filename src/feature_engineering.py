...
class FeatureEngineer:
    ...
    def _crear_rolling_windows(self, df):
        ...
        for nombre_ventana, horas in ventanas.items():
            for evento in eventos_rolling:
                ...
                df[col_rolling] = df.groupby(['id_fabrica', 'id_linea', 'id_maquina_dfos'])[evento].transform(
    ...
    def _crear_tendencias(self, df):
        ...
        for evento in eventos_tendencia:
            ...
            col_12h_prev = df.groupby(['id_fabrica', 'id_linea', 'id_maquina_dfos'])[col_12h].shift(12)
    ...
    def _crear_variabilidad(self, df):
        ...
        for evento in eventos_variabilidad:
            ...
            df[f'{evento_corto}_std_7d'] = df.groupby(['id_fabrica', 'id_linea', 'id_maquina_dfos'])[evento].transform(
                lambda x: x.rolling(window=168, min_periods=24).std()
            ).fillna(0)

            rolling_mean = df.groupby(['id_fabrica', 'id_linea', 'id_maquina_dfos'])[evento].transform(
        ...
        for (fabrica, linea, maquina) in tqdm(df[['id_fabrica', 'id_linea', 'id_maquina_dfos']].drop_duplicates().itertuples(index=False), desc="Máquinas"):
            mask_maquina = (df['id_fabrica'] == fabrica) & (df['id_linea'] == linea) & (df['id_maquina_dfos'] == maquina)
    ...
    def _filtrar_ventanas_prediccion(self, df):
        ...
        idx_ultimo = df.groupby(['id_fabrica', 'id_linea', 'id_maquina_dfos'])['timestamp_hora'].idxmax()
